...
export default function ReportPanel({ apiBase, reportId, onBack }) {
  ...
  // Fetch a specific report, or if null fetch the list
  const fetchData = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      if (reportId) {
        const resp = await fetch(`${apiBase}/reports/${reportId}`)
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
        const data = await resp.json()
        setReport(data)
      } else {
        // Fetch latest 20 reports as overview
    ...
    return (
      <div className="h-full flex flex-col max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="shrink-0 mb-6">
          <h1 className="text-xl font-semibold text-neutral-100 leading-snug">
            {report.title}
          </h1>
          ...
          <div className="shrink-0 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {sources.map((src, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-neutral-900 border border-neutral-800 text-xs text-neutral-400 font-mono"
                >
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          <div className="prose prose-invert prose-sm max-w-none">
            <ReactMarkdown
              components={{
                h1: ({ node, ...props }) => (
                  <h1 className="text-lg font-semibold text-neutral-100 mt-6 mb-3" {...props} />
                ),
                h2: ({ node, ...props }) => (
                  <h2 className="text-base font-semibold text-neutral-100 mt-5 mb-2" {...props} />
                ),
                h3: ({ node, ...props }) => (
                  <h3 className="text-sm font-semibold text-neutral-200 mt-4 mb-2" {...props} />
                ),
                p: ({ node, ...props }) => (
                  <p className="text-sm leading-relaxed text-neutral-300 my-2" {...props} />
                ),
                a: ({ node, ...props }) => (
                  <a
                    {...props}
                ...
                code: ({ node, className, children, ...props }) => {
                  ...
                  if (isInline) {
                    return (
                      <code
                        className="px-1 py-0.5 rounded bg-neutral-800 text-emerald-300 text-[13px] font-mono"
                        {...props}
                      >
                        {children}
                      </code>
                  ...
                  return (
                    <pre className="overflow-x-auto rounded-lg bg-neutral-900 border border-neutral-800 p-3 my-3 text-[13px]">
                ...
                ul: ({ node, ...props }) => (
                  <ul className="text-sm text-neutral-300 space-y-1 my-2 list-disc list-inside" {...props} />
                ),
                ol: ({ node, ...props }) => (
                  <ol className="text-sm text-neutral-300 space-y-1 my-2 list-decimal list-inside" {...props} />
                ),
                blockquote: ({ node, ...props }) => (
                  <blockquote
                    className="border-l-2 border-emerald-500/30 pl-4 italic text-neutral-400 my-3"
                    {...props}
                  />
                ),
                hr: ({ node, ...props }) => (
      ...
      {allReports.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1">
          {allReports.map((r) => (
            <button
              key={r.id}
              onClick={() => window.location.hash = `#/report/${r.id}`}
