...
async def fetch_repo_commits(
    ...
    *,
    ...
    Args:
        ...
        repo: 仓库名称
        per_page: 每页条数，默认 10
        sha: 可选分支名或 commit SHA

    Returns:
        GitHub API 返回的 commit 对象列表
...
async def read_github_file(
    ...
    try:
        content = await _fetch_github_content(
            owner, repo, path, ref=ref or None
        )
        return f"## {owner}/{repo}/{path}\n\n{content}"
    except httpx.HTTPStatusError as e:
        return f"读取文件失败（HTTP {e.response.status_code}）: {owner}/{repo}/{path}"
...
async def save_to_mysql(
    ...
    Args:
        title: 报告标题，简明概括主题
        content: 报告完整正文（Markdown 或纯文本均可）
        query: 用户原始问题（可选）
